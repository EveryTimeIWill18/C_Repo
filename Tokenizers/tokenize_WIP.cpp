...
int* c_vector(int n) {
    /*create a vector of a certain length*/

    int* int_vec = NULL;
    int_vec = (int*)malloc(n*sizeof(int)); // allocate memory
...
/*
 // test string to tokenize
    const char* input_string = "This is a test input string to tokenize";

    // create a pointer of char pointers
    char** token_vector = NULL; // initialize the vector
    // temporary pointer to char that will be overridden after each word
    char* current_word = (char*)malloc(BUFFER*sizeof(char));
    
    int number_of_tokens = 1; // number of words in the string
    int string_length = 0;    // length of the input string
    char delimiter = ' ';     // delimiter used to tokenize

    // get the number of words and number of characetrs
    while(*(input_string + string_length) != '\0'){
        if(*(input_string + string_length) == delimiter) {
            number_of_tokens++;
        }
        string_length++;
    }
    
    printf("Input string length == %d\nNumber of tokens == %d\n", string_length, number_of_tokens); // 39 characters
